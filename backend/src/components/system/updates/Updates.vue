<template>
  ...
  <el-table-column label="Status" width="220">
    <template #default="{ row }">
      <div v-if="installationStatus[row.name]" class="status-container">
...
const installUpdates = async () => {
  try {
    await ElMessageBox.confirm(
      'This will install all available updates automatically. Continue?',
      'Confirm Updates',
      {
    ...
    if (err !== 'cancel') {
      error.value = 'Installation failed: ' + (err.response?.data?.message || err.message)
