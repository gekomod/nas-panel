...
app.get('/system/updates/changelog/:package', requireAuth, async (req, res) => {
  try {
    ...
    try {
      await execPromise(`apt-cache show ${package}`);
    } catch {
    ...
    // Pobierz changelog - użyj apt-get changelog z timeout
    const command = `timeout 30 apt-get changelog ${package} 2>&1 || echo "Changelog not available for ${package}"`;
    const changelog = await execPromise(command);
    
    // Jeśli changelog jest pusty lub zawiera komunikat o błędzie
    if (!changelog || changelog.includes('changelog for') || changelog.includes('not found')) {
      // Spróbuj alternatywną metodę
      const policy = await execPromise(`apt-cache policy ${package}`);
      const version = policy.match(/Candidate: (.+)/)?.[1] || '';
      
      if (version) {
        const altChangelog = await execPromise(`apt-get changelog ${package}=${version} 2>&1 || echo "Changelog not available for ${package}"`);
        return res.json({
          success: true,
          package,
          version,
          changelog: altChangelog
        });
      }
    ...
    // Zamiast 500, zwróć przyjazny komunikat
    res.json({
      success: true,
      package: req.params.package,
      changelog: `# Changelog for ${req.params.package}\n\nUnable to retrieve changelog. Error: ${error.message}`
    });
  }
});
...
app.post('/system/updates/install', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (packages.length > 0) {
      // Użyj DEBIAN_FRONTEND=noninteractive i PTY dla lepszego outputu
      command = `DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ${packages.join(' ')}`;
    } else {
      command = `DEBIAN_FRONTEND=noninteractive apt-get upgrade -y --no-install-recommends`;
    }

    // Użyj pseudo-terminala dla lepszego formatowania outputu
    ...
    const processData = {
      ...
      clients: new Set(),
      output: '',
      fullOutput: '', // Pełny output bez parsowania
      status: 'running'
    };

    activeProcesses.set(processId, processData);

    // Zbieraj pełny output bez modyfikacji
    child.stdout.on('data', (data) => {
      const output = data.toString();
      processData.output += output;
      processData.fullOutput += output;
      
      // Wyślij surowy output do klientów
      processData.clients.forEach(client => {
        try {
          ...
          console.error('Error sending SSE output:', err);
        }
      });
    });
    ...
    child.on('close', (code) => {
      ...
      processData.status = success ? 'success' : 'failed';
      ...
      processData.clients.forEach(client => {
        try {
          client.res.write(`data: ${JSON.stringify({
            ...
            success,
            code,
            message: success ? 'Installation completed successfully' : `Installation failed with code ${code}`,
            timestamp: Date.now()
          })}\n\n`);
          client.res.end();
        } catch (err) {
          console.error('Error sending complete SSE:', err);
        }
      });
      
      // Opcjonalnie: zapisz log do pliku
      const logFile = path.join('/var/log/nas-panel', `install-${processId}.log`);
      try {
        fs.mkdirSync(path.dirname(logFile), { recursive: true });
        fs.writeFileSync(logFile, processData.fullOutput);
      } catch (err) {
        console.error('Error saving install log:', err);
      }
      
      // Usuń proces po 5 minutach
      setTimeout(() => {
...
app.get('/system/updates/progress/:processId', requireAuth, (req, res) => {
  ...
  // Wyślij cały dotychczasowy output
  if (process.fullOutput) {
    res.write(`data: ${JSON.stringify({
      type: 'output',
      data: process.fullOutput,
      isInitial: true,
      timestamp: Date.now()
    })}\n\n`);
  }
  ...
  req.on('close', () => {
    ...
    // Jeśli proces nadal działa i nie ma klientów, oznacz go jako orphaned
    if (process.clients.size === 0 && process.status === 'running') {
      console.log(`Process ${processId} has no more clients, but continues running`);
    }
  });
});

app.get('/system/updates/log/:processId', requireAuth, (req, res) => {
  const { processId } = req.params;
  const process = activeProcesses.get(processId);
  
  if (!process) {
    // Spróbuj odczytać z pliku
    const logFile = path.join('/var/log/nas-panel', `install-${processId}.log`);
  ...
  res.setHeader('Content-Type', 'text/plain');
  res.send(process.fullOutput || process.output || 'No output available');
});
