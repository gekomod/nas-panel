  ...
  app.get('/network/interfaces/details/:interface', requireAuth, async (req, res) => {
    try {
      ...
      try {
        ...
        console.error(`Error getting config for ${iface}:`, e);
      }
...
async function runIperfTest(host, port, reverse = false) {
  ...
  try {
    ...
    console.error(`iperf3 test failed on ${host}:${port}`, error);
    throw error;
  }
}
...
app.get('/network/firewall/rules', requireAuth, async (req, res) => {
  try {
    ...
    // Debugowanie - zapisz surowe dane do logów
    console.log('Raw ufw output:', stdout);
