...
async function executeDockerBackup(job) {
  try {
    ...
    if (!fsa.existsSync(backupDir)) {
      fsa.mkdirSync(backupDir, { recursive: true });
    }

    // Komendy do wykonania backupu
    const commands = [];
    ...
    if (job.includes.includes('volumes')) {
      ...
      const volumes = fsa.readdirSync('/var/lib/docker/volumes');
      if (volumes.length > 0) {
        commands.push(`docker run --rm -v ${volumeBackup}:/backup -v ${volumes.map(v => `/var/lib/docker/volumes/${v}:/volume/${v}`).join(' -v ')} alpine tar -czf /backup /volume`);
      }
    }
...
app.post('/services/docker/backup/schedule', requireAuth, (req, res) => {
  try {
    ...
    const jobConfig = {
      ...
      location,
      includes,
      command: `docker-backup-${Date.now()}`,
